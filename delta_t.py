...
compare_command = "./compare_gal_files/compare_gal_files 3 result.gal ref_output_data/sun_and_planets_verlet.gal"

# nsteps and delta_t values
nsteps_values = [20*i for i in range(1, 20)]
delta_t_values = [1e-3 / i for i in range(1, 20)]
...
plt.xlabel('Delta t', fontsize=18, fontweight='bold')
plt.ylabel('pos_maxdiff', fontsize=18, fontweight='bold')
plt.tick_params(axis='both', which='major', labelsize=16)
plt.grid(True)  # Add grid

# Change the name here if you modify N, else it would overwrite the previous plot
plt.savefig('delta_t_Vervet.png', dpi=300, bbox_inches='tight')
plt.legend()
plt.show()
