...
int main(int argc, const char* argv[]) {
    int N = 3;
    const char* fileName1 = "N3.gal";

    FILE* error_file = fopen("error_values_order2.txt", "w");
    if (!error_file) {
        printf("Error opening file 'error_values_order2.txt'\n");
        return -1;
    }
    ...
    for (int i = 0; i < 20; i++) {
        char fileName2[50];
        sprintf(fileName2, "results_N3_order2/result_%d.gal", i);
        double buf1[6 * N];
